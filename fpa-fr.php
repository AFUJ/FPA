...
function date_fr($format, $timestamp=false) {
	...
	/** DEFINIR LE FPA PAR DEFAUT *****************************************************************/
	// define ( '_FPA_BRA', TRUE );      // Mode Rapport de bogue, sinon c'est l'assistant aux messages du forum joomla.fr
	...
	define ( '_VALID_MOS', 1 );         // pour J!1.0
	define ( '_JEXEC', 1 );             // pour J!1.5, J!1.6, J!1.7, J!2.5, J!3.0
	...
	define ( '_last_updated', '01/10/2014' );
	define ( '_COPYRIGHT_STMT', ' Copyright (C) 2011, 2012 Russell Winter, Phil DeGruy, Adaptation et traduction pour joomla.fr par Yann Gomiero &nbsp;' );
	...
	define ( '_FPA_DBTBL_TITLE', 'Structure de Table' );
	...
	define ( '_FPA_INS_4', 'Select detail level options of output <i>(option)</i>' );
...
<p align="left" />
  <div align="left"><font size="2">4.) Utiliser CTRL-v pour coller le texte copié dans votre message du forum à l\'endroit désiré.</font></div>' ); //added instruction lines Phil 4-20-12
	...
	define ( '_FPA_3PD', 'Element Tierce' );
	...
	define ( '_FPA_CRE', 'Créé' ); // added this missing variable define 4-8-12 - Phil
...
// Something went wrong and the script was not deleted so it must be removed manually so we tell the user to do so - Phil 8-07-12
	if (file_exists($filename)) {
		...
		echo "<p><font color='#FF0000' size='4'>Oups!</size></font color>";
...
?>
		...
		echo '<p>FPA mis à jour le : '. _last_updated . '</p>' ;
		?></title>
		...
		function toggle2(showHideDiv, switchTextDiv) {
			...
			if ( ele.style.display == "block" ) {
				...
				text.innerHTML = "<span style=\"font-size:12px;color:#800000;\"><span style=\"font-size:20px;color:#800000;\">&otimes;</span> Cacher <strong><?php echo _RES; ?></strong></span>";
			}
		}
		</script>
						...
						<br />
							...
							<?php
								if ( @$_POST['showElevated'] ) {
									...
									$selectshowElevated = ''; // changed to checked - Phil - 4-20-12
								}
							...
							?>
								...
								<strong>Information Privée :</strong><br />
			...
			if ( @$_POST['doIT'] == '1' ) {
				...
				*****************************************************************************************/
				...
				echo "\n";
							...
							echo "\n";
								...
								echo 'MySQL '. _FPA_CFG .' ::  ';
									...
									if ( @$database['dbERROR'] != _FPA_N ) {
										...
										echo _FPA_VER .': '. $database['dbHOSTSERV'] .' (Client:'. $database['dbHOSTCLIENT'] .') | '. _FPA_TCOL .': '. $database['dbCOLLATION'] .' ('. _FPA_CHARS .': '. $database['dbCHARSET'] .') | '. _FPA_TSIZ .' '. _FPA_DB .' : '. $database['dbSIZE'] .' | # '. _FPA_OF .' '. _FPA_TBL .' : '. $database['dbTABLECOUNT'];
									}
					...
					echo "\n";
					echo "\n";
					echo _RES .' (v'. _RES_VERSION .') : '. @date_fr("d F Y");
				...
				echo "\n";
							...
							echo "\n";
								...
								echo '> <sub>**MySQL '. _FPA_CFG .' ::** ';
									...
									if ( @$database['dbERROR'] != _FPA_N ) {
										...
										echo '**'. _FPA_VER .':** '. $database['dbHOSTSERV'] .' (Client:'. $database['dbHOSTCLIENT'] .') | **'. _FPA_TCOL .':** '. $database['dbCOLLATION'] .' (**'. _FPA_CHARS .':** '. $database['dbCHARSET'] .') | **'. _FPA_TSIZ .' de '. _FPA_DB .':** '. $database['dbSIZE'] .' | **# '. _FPA_OF .' '. _FPA_TBL .':** '. $database['dbTABLECOUNT'] .'</sub>';
									}
					...
					if ( $_POST['probDSC'] ) { echo '[quote="'. _FPA_PROB_DSC .' :: '. _RES .' (v'. _RES_VERSION .') : '. @date_fr("d F Y") .'"][size=2]'. $_POST['probDSC'] .' [/size][/quote]'; }

					if ( $_POST['probMSG1'] ) { echo '[quote="'. _FPA_PROB_MSG .' :: '. _RES .' (v'. _RES_VERSION .') : '. @date_fr("d F Y") .'"][size=2]'. $_POST['probMSG1'] .'[/size][/quote]'; }

					if ( $phpenv['phpLASTERR'] AND $_POST['probMSG2'] ) { echo '[quote="'. _FPA_LAST .' PHP '. _FPA_ER .' :: '. _RES .' (v'. _RES_VERSION .') : '. @date_fr("d F Y") .'"][size=2][color=#800000]'. $_POST['probMSG2'] .'[/color][/size][/quote]';
					} elseif ( !@$phpenv['phpLASTERROR'] AND $_POST['probMSG2'] ) { echo '[quote="'. _FPA_PROB_MSG .' :: '. _RES .' (v'. _RES_VERSION .') : '. @date_fr("d F Y") .'"][size=2]'. $_POST['probMSG2'] .'[/size][/quote]'; }

					if ( $_POST['probACT'] ) { echo '[quote="'. _FPA_PROB_ACT .' '. _FPA_BY .' '. _RES .' (v'. _RES_VERSION .') '. @date_fr("d F Y") .'"][size=2]'. $_POST['probACT'] .'[/size][/quote]'; }

					echo  'Configuration détaillé par le script FPA - '._RES .' (v'. _RES_VERSION .') : '. @date_fr("l d F Y");
					echo "\r\n";

					// do the basic information
					echo '[quote="Environnement sommaire ::"][size=2]';
...
// fixed minor syntax error - Phil 09-23-12
							echo '[b]'. _FPA_TCOL .':[/b] '. $database['dbCOLLATION'] .' ([b]'. _FPA_CHARS .':[/b] '. $database['dbCHARSET'] .') | [b]'. _FPA_TSIZ .' de '. _FPA_DB .':[/b] '. $database['dbSIZE'] .' | [b]#'. _FPA_OF .'&nbsp'. _FPA_TABLE .':&nbsp[/b] '. $database['dbTABLECOUNT'];
					}
		...
		echo '[/size][/quote]';
					...
					echo '[quote="Informations détaillés ::"][size=2]';
					...
					echo '[/size][/quote]';
						...
						if ( $instance['instanceFOUND'] == _FPA_Y ) {
							echo '[quote="Droits sur répertoire ::"][size=2]';
								...
								echo '[color=#000000][b]'. _FPA_COREDIR_TITLE .' :: [/b][/color]';
									...
									if ( @$_POST['showElevated'] == '1' ) {
										...
										$limitCount = '0';
										echo '[color=#000000][b]'. _FPA_ELEVPERM_TITLE .'[/b] [i](les 10 '. _FPA_FIRST .')[/i][b] :: [/b] [/color]';
					...
					if ( $database['dbDOCHECKS'] == _FPA_Y AND @$database['dbERROR'] == _FPA_N AND @$_POST['showTables'] == '1' ) {
						echo '[quote="Information base de données ::"][size=2]';
...
//                                }
					...
					echo '[quote="Extensions identifiées ::"][size=2]';

						if ( $_POST['showProtected'] == '3' ) {
							echo '[color=orange][b]Strict[/b] l\'Information est privée.[/color] Aucun renseignement ne sera communiqué.';
							echo '[/size][/quote]';
						} else {
...
//                        }
						...
						if ( $instance['instanceFOUND'] == _FPA_Y ) {
							echo '[quote="'. _FPA_TMPL_TITLE .' identifiés ::"][size=2]';
...
<?php
	...
	echo '<div class="half-section-container" style="">'; // start half-section container
		...
		echo '<div class="mini-content-box-small" style="">';
		echo '<div style="line-height:10px;font-size:8px;color:#404040;text-shadow: #fff 1px 1px 1px;width:99%;border-bottom: 1px solid #ccebeb;font-weight:bold;padding:1px;padding-top:0px;padding-right:0px;padding-bottom:2px;text-transform:uppercase;">Dernière erreur connue en PHP  :<div style="float:right;">';
...
<?php
		if ( $phpenv['phpAPI'] == 'apache2handler' ) {
			...
			/** display the instance information *************************************************/
			echo '<div class="section-information">'; // start right content block
			echo '<div class="section-title" style="text-align:center;">'. $apachemodules['ARRNAME'] .' :: Détails</div>';
			echo '<div class="" style="width:99%;margin: 0px auto;clear:both;margin-bottom:10px;">';
...
<?php
	...
	if ( $showElevated == '1' ) {
		...
		echo '<div class="section-information">';
		echo '<div class="section-title" style="text-align:center;">'. $elevated['ARRNAME'] .' (les 10 '. _FPA_FIRST .')</div>';
...
<?php
	...
	if ( defined( '_FPA_DEV' ) ) {
		...
		// SELECTIONS
		echo '<div style="font-weight:bold;width:25%;float:left;text-align:center;">Mode développeur<br />';
			if ( defined ( '_FPA_DEV' ) ) {
